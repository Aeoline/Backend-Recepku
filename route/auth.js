          ...
          db.collection("users")
            ...
            .then((doc) => {
              if (doc.empty) {
                ...
                console.log(
                  `email ${doc.docs[0].data().email} sudah terdaftar`
                );
                return res.status(500).json({
                  error: true,
        ...
        db.collection("users")
          ...
          .then((doc) => {
            if (doc.empty) {
              ...
              return res
                .status(500)
                .send("username atau email tidak terdaftar");
            } else {
              bycript.compare(
                data.password,
                doc.docs[0].data().password,
                (err, result) => {
                  if (result) {
                    var token = jwt.sign(
                      {
                        ...
                        isAdmin: doc.docs[0].data().isAdmin,
                      },
                      secretKey,
                      { expiresIn: "1h" }
                    );
        ...
        bycript.compare(
          ...
          (err, result) => {
            if (result) {
              ...
              req.session.email = doc.docs[0].data().email;
              req.session.image_url = doc.docs[0].data().image_url;
              req.session.isAdmin = doc.docs[0].data().isAdmin;
              console.log("Welcome " + req.session.username);
              return res.status(200).json({
                error: false,
                message: "Welcome " + req.session.username,
